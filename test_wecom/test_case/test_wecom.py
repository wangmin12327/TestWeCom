...
from test_wecom.page_object.get_data import get_data
from test_wecom.page_object.login_page import LoginPage
...
class TestWeCom:
    ...
    def test_add_member(self, name, account, phone_number, business_mail):
        ...
        with allure.step("打印通讯录页面的所有成员及对应账号、手机号、邮箱"):
            print(res)

        with allure.step("多断言：所有测试数据中的成员姓名、账号、手机号和邮箱是否包含在新增成员信息列表中"):
            assert name in res
            sleep(3)
